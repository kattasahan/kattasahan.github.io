...
import { LinkButton, Tag } from '@portfolio/ui'
...
function ArticleContent({ routeName }: { routeName: NotesArticleRoute | undefined }) {
  ...
  if (!article) {
    return (
      <section aria-labelledby="not-found-title" className="article-column">
        ...
        <LinkButton href={route('notes')} theme={theme} variant="secondary">
          Return to Notes
        </LinkButton>
      </section>
    )
  ...
  return (
    <article className="article-column">
      <LinkButton href={route('notes')} theme={theme} variant="ghost">
        ← All notes
      </LinkButton>
      <header style={{ marginTop: spacingTokens[3] }}>
        ...
        <h1
          style={{
            fontSize: `clamp(${typographyTokens.fontSize['4xl']}, 7vw, ${typographyTokens.fontSize['6xl']})`,
      ...
      >
        ...
        <h2
          style={{ fontSize: typographyTokens.fontSize['2xl'], margin: `${spacingTokens[1]} 0 0` }}
        >
          {nextArticle.title}
        </h2>
        <LinkButton
          href={articleHref(nextArticle.route)}
