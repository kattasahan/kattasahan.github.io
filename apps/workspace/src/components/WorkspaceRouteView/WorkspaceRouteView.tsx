...
import { Card, LinkButton } from '@portfolio/ui'
...
export function WorkspaceRouteView({
  ...
  if (isLoading) {
    return (
      ...
      >
        <Card theme={theme}>
          <p style={{ color: color.textMuted, margin: 0 }}>Loading placeholder</p>
        </Card>
      </SectionLayout>
    )
  }
  ...
  if (!routeName) {
    return (
      <SectionLayout
        description="This route is not part of the Workspace experience."
        eyebrow="404"
  ...
  return (
    <SectionLayout
      description={definition?.description ?? 'A Workspace route placeholder.'}
      eyebrow={definition?.label}
      title={
        caseStudySlug
          ? `${definition?.title}: ${caseStudySlug}`
          : (definition?.title ?? 'Workspace')
      }
    >
      <Card theme={theme}>
        <p
        ...
        >
          This shared shell is ready for page content. The final experience will add content without
          changing the surrounding navigation, layout, loading, or error states.
        </p>
      </Card>
    </SectionLayout>
  )
