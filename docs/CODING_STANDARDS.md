...
- Use `@portfolio/routes` for application runtime route constants and resolution. Vite configuration must use the Node-loadable `@portfolio/routes/config` entrypoint. `@portfolio/config` route entries are temporary compatibility re-exports for existing imports only.
