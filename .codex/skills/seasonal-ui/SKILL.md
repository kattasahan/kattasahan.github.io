...
- Show a compact `ACTIVE` status at the profile card’s top-left: a coral dot with a restrained Tailwind ping animation and `motion-reduce:animate-none` support. Use a container query for the identity header: stack the portrait above name, role, and location below 288px of available card content; otherwise present them in a horizontal row.
