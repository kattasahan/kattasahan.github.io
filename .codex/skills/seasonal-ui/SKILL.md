...
- Keep Inter Tight non-blocking: load its stylesheet from `index.html` with a print-media swap plus a `noscript` fallback, never through a CSS `@import`. This avoids delaying the first render while retaining the requested typeface.
